...
if(mqttClient.publish("/weathersensor1/temperature", charBuf))

mqttData = String(totalRainfall);
mqttData.toCharArray(charBuf, 50) ;
if(mqttClient.publish("/weathersensor1/rainfall", charBuf))

mqttData = String(bme.getHumidity());
mqttData.toCharArray(charBuf, 50) ;
if(mqttClient.publish("/weathersensor1/humidity", charBuf))

mqttData = String(bme.getPressure_MB());
mqttData.toCharArray(charBuf, 50) ;
if(mqttClient.publish("/weathersensor1/pressure", charBuf))

mqttData = String(windSpeed);
mqttData.toCharArray(charBuf, 50) ;
if(mqttClient.publish("/weathersensor1/wind_speed", charBuf))

getWindDirection();
mqttData = String(calDirection);
mqttData.toCharArray(charBuf, 50) ;
if(mqttClient.publish("/weathersensor1/wind_direction", charBuf))

mqttData = String(lux);
mqttData.toCharArray(charBuf, 50) ;
if(mqttClient.publish("/weathersensor1/luminosity", charBuf))
